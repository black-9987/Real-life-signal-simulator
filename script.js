...
const els = {
    ...
    fdevVal: document.getElementById('fdev-val'),
    ...
    betaStatus: document.getElementById('beta-status'),
...
function getColors() {
    ...
    return {
        ...
        fm: root.getPropertyValue('--color-fm').trim(),
...
function initPlots() {
    ...
    // FM Plot
    Plotly.newPlot('fm-plot', [{
        x: time, y: [], type: 'scatter', mode: 'lines',
        line: { color: c.fm, width: 1.5, shape: 'spline' },
        name: 's_FM(t)'
    }], getPlotLayout([-6, 6]), plotConfig);
}
...
function updateCalculations() {
    ...
    const beta = state.fdev / state.fm;
    els.betaDisplay.textContent = beta.toFixed(2);
    
    if (beta < 0.5) {
        els.betaStatus.textContent = 'Narrowband FM';
        els.betaStatus.className = 'mu-status status-under';
        els.betaDisplay.style.color = 'var(--success)';
    } else {
        els.betaStatus.textContent = 'Wideband FM';
        els.betaStatus.className = 'mu-status status-over';
        els.betaDisplay.style.color = 'var(--accent-color)';
    }
}
...
function generateSignals() {
    ...
    const fm = new Float32Array(POINTS);
    ...
    for (let i = 0; i < POINTS; i++) {
        ...
        fm[i] = state.ac * Math.sin(w_c * t - beta * Math.cos(w_m * t));
        envPos[i] = envelope;
        envNeg[i] = -envelope;
    }
    
    return { msg, car, am, fm, envPos, envNeg };
}
...
function updatePlots() {
    ...
    Plotly.restyle('fm-plot', { y: [sigs.fm] });
}
...
els.resetBtn.addEventListener('click', () => {
    state = { ...state, ac: 1.0, am: 0.5, fc: 20, fm: 2, fdev: 10, timeOffset: 0 };
...
window.addEventListener('resize', () => {
    ['message-plot', 'carrier-plot', 'am-plot', 'fm-plot'].forEach(id => {
